function [fused_eval, rc_eval, wb_eval, gc_eval, sh_eval] = main(img_path, plot_output)
%{
    Description: The MAIN function performs the whole fusion algorithm on a
    given image, and outputs the evaluations of its channel compensated,
    white-balanced, gamma corrected, sharpened, and the recovered version
    independently.

    Input:
        - img: an RGB image (a mxnx3 tensor)
        - plot_output: a 0 or 1 indicating if plotting the results is
        necessary
    Output:
        - fused_eval: the evaluation of the fused pyramid
        - rc_eval: the evaluation of the red compensated image
        - wb_eval: the evaluation of the white-balanced image
        - gc_eval: the evaluation of the gamma corrected image
        - sh_eval: the evaluation of the sharpened image
%}
    img = imread(img_path);
